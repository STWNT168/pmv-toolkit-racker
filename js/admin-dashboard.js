const AdminDashboard = (() => {
  ...
  async function load(date) {
    ...
    if (status) { status.textContent = "Fetching dashboard data…"; status.className = "admin-status"; }

    try {
      const x = await AdminDashboardApi.getUpdateStatus(date);
      render(x);
      if (status) {
        status.textContent = `Data fetched successfully · ${new Date().toLocaleTimeString()}`;
        status.className = "admin-status admin-status-success";
      }
    } catch (e) {
      if (status) { status.textContent = e.message || "Unable to load dashboard."; status.className = "admin-status admin-status-error"; }
    } finally {
  ...
  function render(d) {
    ...
    if (officeRows) {
      ...
      (d.officeWise || []).forEach(x => {
        ...
        [
          ...
          x.allKits ?? 0,x.similarArticle ?? 0,x.deliverableKits ?? 0,
          x.invalidMobileKits ?? 0,x.incompleteKits ?? 0,x.withoutProperDetailsKits ?? 0,
          x.deliverableArticles ?? 0,x.invalidMobileArticles ?? 0,
          x.incompleteArticles ?? 0,x.withoutProperDetailsArticles ?? 0
        ].forEach(v => { const td=document.createElement("td"); td.textContent=String(v); tr.appendChild(td); });
        officeRows.appendChild(tr);
      });
      if (!officeRows.children.length) officeRows.innerHTML='<tr><td colspan="15">No active SPM offices found.</td></tr>';
    }
    ...
    if (pendingRows) {
      ...
      (d.pendingSpms || []).forEach((x,i) => {
        const tr=document.createElement("tr");
        [i+1,x.spmName,x.spmId,x.officeName,"Not updated"].forEach(v=>{const td=document.createElement("td");td.textContent=String(v||"—");tr.appendChild(td);});
        pendingRows.appendChild(tr);
      });
      if (!pendingRows.children.length) pendingRows.innerHTML='<tr><td colspan="5">All active SPMs have updated.</td></tr>';
    }
